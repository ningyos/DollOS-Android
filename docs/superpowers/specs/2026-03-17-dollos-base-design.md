...
1. **DollOS Base** -- AOSP 建構流程、裝置支援（本文件）
...
|----------|--------|-----------|
| AOSP 基底 | 純 AOSP 16 | 官方 Pixel 裝置支援、架構簡潔、維護成本低 |
| 目標裝置 | Pixel 6 以上 (Tensor 晶片) | 開發機為 Pixel 6a (bluejay)，Tensor 系列硬體一致 |
| GMS 策略 | 不內建 GMS（使用者可自行安裝 microG 或 GMS APK） | DollOS 核心功能不依賴 GMS，降低複雜度 |
...
基於純 AOSP 16 作為 DollOS 的基底，使用 Google 官方 manifest。

**版本策略：** 追蹤 AOSP 16 的官方 release tag（例如 `android-16.0.0_r2`）。

**已知風險：** Google 可能在未來的 Android 版本中調整 Pixel 6 系列的支援。DollOS 應及早擴展測試到 Pixel 8+ 裝置以降低風險。

### Repository Structure

DollOS 使用 AOSP 官方 `repo` manifest，搭配 `local_manifests` 覆蓋/新增 DollOS 專屬的 repo：

- **不 fork 整個 manifest** — 直接使用 AOSP 官方 manifest
- 需要修改的 repo（如 `frameworks/base`）在 `local_manifests` 中覆蓋為 DollOS 的 fork
- DollOS 專屬的 repo（DollOSService、SetupWizard、vendor、device）透過 `local_manifests` 引入
- DollOS 的 GitHub 組織：`github.com/ningyos/`

### Build Environment

- **建構主機：** Linux (Ubuntu 24.04 LTS)
- **建構系統：** 沿用 AOSP 標準建構腳本和工具鏈
...
- 追蹤 AOSP 官方 release tag，定期更新到最新的安全修補版本
...
DollOS 不內建 Google Mobile Services (GMS)。使用者可自行選擇：

- 安裝 microG（開源 GMS 替代品）
- 手動安裝 Google Play Services APK
- 完全不使用 GMS
...
- DollOS 自身的元件均使用開源方案
- GMS 僅為使用者提供 Play Store 和第三方 app 相容性

### Default State

- 首次開機的 OOBE 中不提供 GMS 安裝選項（簡化流程）
- 使用者可在設定完成後自行安裝 microG 或 GMS
